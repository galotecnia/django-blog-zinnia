...
def do_blog_context(extra_context, kwargs):
    """Add blog and filters to context (if needed)"""
    blog_slug = ''
    if ZINNIA_BLOG_ACTIVE:
        blog_slug = kwargs.pop('blog_slug')
        blog = get_object_or_404(Blog, slug = blog_slug)
        extra_context.update({'blog': blog, 'filters': {'blog__slug': blog_slug}})
    return blog_slug
...
def category_detail(request, slug, page=None, **kwargs):
    ...
    blog_slug = do_blog_context(extra_context, kwargs)
